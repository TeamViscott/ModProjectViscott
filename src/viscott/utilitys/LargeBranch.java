...
import viscott.content.BranchFx;

import static mindustry.Vars.*;

/** another modified copy of Lightning class, with alterations.
 * @author Otamamori
 */


public class LargeBranch {
    ...
    private static void createBranchInternal(@Nullable Bullet hitter, int seed, Team team, float damage, float x, float y, float rotation, int length){
        ...
        BranchFx.branch.at(x, y, rotation, lines);
        float finalRotation = rotation;
        float finalX = x;
        float finalY = y;
        Time.run(BranchFx.branch.lifetime, () -> {
            BranchFx.branchFade.at(finalX, finalY, finalRotation, lines);
