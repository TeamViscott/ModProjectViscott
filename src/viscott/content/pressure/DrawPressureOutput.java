...
public class DrawPressureOutput extends DrawBlock {
    public TextureRegion heat, glow, top1, top2;

    public Color heatColor = new Color(1f, 1f, 1f, 0.8f);
    public float heatPulse = 0.3f, heatPulseScl = 10f, glowMult = 1.2f;

    public int rotOffset = 0;
    public boolean drawGlow = true;

    public DrawPressureOutput(){}

    public DrawPressureOutput(int rotOffset, boolean drawGlow){
    ...
    public void draw(Building build){
        ...
        if(build instanceof PressureBlock heater && heater.pressure() > 0){
            Draw.z(Layer.blockAdditive);
            Draw.blend(Blending.additive);
            Draw.color(heatColor, heater.pressureFraction() * heater.pressure() * (1f - heatPulse + Mathf.absin(heatPulseScl, heatPulse)));
